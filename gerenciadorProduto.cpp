...
GerenciadorProduto::GerenciadorProduto() {
    ...
    void GerenciadorProduto::exibirRelatorio() const {
        ...
        std::cout << "\tQuantidade de Produtos: " << produtos.size() << std::endl;
        float valorTotal = 0.0;
        for (const auto& produto : produtos) {
            valorTotal += produto->calcularValorTotal();
        }
        std::cout << "\tValor Total do Estoque: " << valorTotal << std::endl;
    }
    ...
    if (produto != nullptr) {
        if (produto->getQuantidade() >= quantidadeVendida) {
            ...
            std::cout << "Quantidade insuficiente em estoque para a venda." << std::endl;
        }
    } else {
